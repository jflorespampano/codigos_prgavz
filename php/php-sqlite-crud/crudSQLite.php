...
class ProvPar extends DataBase
{
    ...
    function actualizaDato($tipoParte)
    {
        ...
        # primera forma de ligar parametros
        // $id = $tipoParte["id"];
        // $descripcion = $tipoParte["descripcion"];
        ...
        // $resultado = $sentencia->execute();
        #segunda forma
        $resultado = $sentencia->execute($tipoParte);
        $baseDeDatos = null;
        if ($resultado === true) {
            http_response_code(200);
            return true;
    ...
    {
        $baseDeDatos = $this->abrirDB();
        $sentencia = $baseDeDatos->prepare("DELETE FROM tipo_parte WHERE id=:id;");
...
function get()
{
    if (!empty($_GET)) {
        ...
        $objPP = new ProvPar();
        $data = $objPP->consultaTipoPartes("select * from tipo_parte where id=" . $id);
        echo $data;
    } else {
        //devolvemos todos
        $objPP = new ProvPar();
        $data = $objPP->consultaTipoPartes("select * from tipo_parte");
        echo $data;
    }
}
